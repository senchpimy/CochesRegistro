...
func (c *Coche) Random() {
	...
	marcas := []string{"Toyota", "Ford", "Honda", "Nissan", "Chevrolet"}
	modelos := []string{"Camry", "Mustang", "Civic", "Sentra", "Cruze"}
	colores := []string{"Rojo", "Azul", "Negro", "Blanco", "Plata"}
	tamaños := []string{"Grande", "Mediano", "Pequeño"}

	c.marca = marcas[rand.Intn(len(marcas))]
	c.modelo = modelos[rand.Intn(len(modelos))]
	c.año = rand.Intn(23) + 2000
	c.color = colores[rand.Intn(len(colores))]
	c.tamaño = tamaños[rand.Intn(len(tamaños))]
}
...
func (l *List) InsertarCoche(c Coche) {
	newNode := &Node{coche: c, Next: nil}
	if l.Head == nil {
		l.Head = newNode
		return
	}
...
func (l *List) EliminarCoche(c Coche, vendidos *List) bool {
	if l.Head == nil {
		return false
	}
	if l.Head.coche.marca == c.marca && l.Head.coche.modelo == c.modelo && l.Head.coche.año == c.año {
		nodoEliminado := l.Head
		l.Head = l.Head.Next
		nodoEliminado.Next = nil
		vendidos.InsertarNodo(nodoEliminado)
		return true
	}
	p := l.Head
	for p.Next != nil && (p.Next.coche.marca != c.marca || p.Next.coche.modelo != c.modelo || p.Next.coche.año != c.año) {
		p = p.Next
	}

	if p.Next != nil {
		nodoEliminado := p.Next
		p.Next = p.Next.Next
		nodoEliminado.Next = nil
		vendidos.InsertarNodo(nodoEliminado)
		return true
...
func (l *List) Obtener(n int) *Node {
	p := l.Head
	for i := 0; i < n && p != nil; i++ {
		p = p.Next
	}
...
func ComprarCoche(c *Coche, disponibles, comprados *List) error {
	if disponibles.Head == nil {
		return errors.New("No hay lugares disponibles")
	}
	lugar := disponibles.Head
	disponibles.Head = disponibles.Head.Next
	fmt.Printf("Se ha comprado el coche en el lugar #%d\n", lugar.coche.año)
	comprados.InsertarCoche(*c)
	return nil
}

func Comprar(año int, color, marca, modelo, tamaño string, disponibles, comprados *List) error {
	coche := Coche{año: año, color: color, marca: marca, modelo: modelo, tamaño: tamaño}
	return ComprarCoche(&coche, disponibles, comprados)
}
